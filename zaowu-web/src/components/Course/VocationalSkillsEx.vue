  ...
  <div class="course">
    <div class="container clearfix">
      <el-tabs :tab-position="'left'" style="height:800px">
        <el-tab-pane class="tab" label="职业技能考试">
          <ul
            class="infinite-list"
            ...
            <div class="intro">
              <div class="intro">
                ...
                <el-steps :active="5" align-center style="margin-top:3%">
                  <el-step title="社会工作者"></el-step>
                  <el-step title="考试专业选择"></el-step>
                  <el-step title="官网报名"></el-step>
                  <el-step title="获得证书"></el-step>
                </el-steps>
                <img :src="aboutEx.image" alt="" class="introImg" />
                <p class="introDes">{{ aboutEx.subDescription1 }}</p>
                <p class="introDes">{{ aboutEx.subDescription2 }}</p>
                <p class="introDes">{{ aboutEx.subDescription3 }}</p>
              </div>
            </div>
          </ul>
        </el-tab-pane>
            ...
            <el-form-item label="微信" prop="wechat">
             ...
             <el-form-item label="考试科目" prop="exSubject">
              <el-select style="width: 100%" v-model="applyForm.exSubject" placeholder="请选择考试科目">
                <el-option
                  v-for="(item,index) in exSubjectOptions"
                  :key="index+'ss'"
                  :label="item.label"
                  :value="item.value">
                </el-option>
...
<script>
import skills_bg from '../../assets/skills_bg.png'
import about_ex from '../../assets/skills_about.png'
export default {
  name: "Course",
  data() {
    ...
    return {
      ...
      aboutEx: {
        title: '职业技能证书用途',
        subDescription1: '1.国家职业资格证书制度是指按照国家制定的职业技能标准或任职资格条件，通过认定的考核鉴定机构，对劳动者的技能水平或职业资格进行客观公正、科学规范的评价和鉴定，对合格者授予相应的国际职业资格证书。',
        subDescription2: '2.国家职业资格证书分为五个等级：五级（初级）、四级（中级）， 三级（高级）、二级（技师）和一级（高级技师）,它是表明劳动者具有某一职业所必备的学识和技能的证明，它是技能水平的凭证，也与工资福利待遇挂钩。',
        subDescription3: '3.职业资格证书是各类院校毕业生求职、任职，开业和上岗的资格凭证，是用人单位招聘，录用劳动者的主要依据，也是境外就业，对外劳务合作人员办理技能水平公正的有效证件',
        image: about_ex
      }
    };
  },
  methods: {
    ...
    //     .catch(err => {});
        const introList = [
            {
                image: skills_bg,
                description1: "职业技能考试，是一项基于职业技能水平的考核活动，属于标准参照型考试。它是由考试考核机构对劳动者从事某种职业所应掌握的技术理论知识和实际操作能力做出客观的测量和评价。",
                description2: "书证相互衔接融通是“1+参加职业技能考试，首先要根据自身职业的资格条件，如果需要培训，要到经政府有关部门批准的培训机构参加培训。考试时要准备好照片、身份证以及证明自己资历的材料，参加正规培训的须有培训机构证明，工作年限须有本人所在单位证明，经考试机构审查符合要求的，由相机构颁发准考证。参加考试时必须携带准考证，否则不能参加考试。",
                title: "什么是职业技能考试"
            },
